...
int main() {
    int n = 2;  // Example dimension
    int m = 2;   // Number of assets
    double alpha = 1;  // Example value of alpha
    std::vector<int> omega_l = {0,1};  // Example mask omega_l: select the states of interest
    ...
    for (int i = 0; i < n; ++i) {
        auto p_square_i = M->variable(Domain::greaterThan(0.0)); // Auxiliary variable for p[i]^2
        M->constraint("cone_" + std::to_string(i), Expr::sub(p_square_i, p->index(i)), Domain::equalsTo(0.0)); // p_square_i = p[i]
    ...
    for (size_t i = 0; i < omega_l.size(); ++i) {
        int idx = omega_l[i];
        obj_expr = Expr::add(obj_expr, Expr::add(Expr::mul(p->index(idx), 1.0), Expr::mul(q->index(idx), 1.0)));
    }
